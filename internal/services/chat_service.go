...
func (s *ChatService) GetOrCreateDirect(ctx context.Context, userID, targetUserID string, req *models.CreateChatRequest) (*models.Chat, error) {
	chat, err := s.chatRepo.FindDirectChat(ctx, userID, targetUserID, req.IsSecret)
	if err == nil {
		return chat, nil
	}
	return s.chatRepo.Create(ctx, "direct", userID, nil, []string{targetUserID}, req.IsSecret, req.MessageTTL)
}
...
func (s *ChatService) CreateGroup(ctx context.Context, userID string, req *models.CreateChatRequest) (*models.Chat, error) {
	...
	return s.chatRepo.Create(ctx, req.Type, userID, req.Name, req.MemberIDs, req.IsSecret, req.MessageTTL)
}
